...
**登場**: マーカーを認識したとき、モデルが名刺の中からせり上がる（1.4秒、ease-out）。
**見失って再取得したときも、向きをリセットして最初から再生する。** ただし
トラッキングは細かくちらつくので、0.5秒以上見失ったときだけ再生する
（毎回のちらつきで再生するとストロボになる）。名刺の平面にクリッピング平面を張り、まだ面の下にある部分を描かない
ことで「中から出てくる」ように見せている。平面はマーカーの姿勢から毎フレーム
作り直す（`renderer.localClippingEnabled`）。

**操作**: 人差し指の先端だけで触れて、なでると回る。回転軸は**名刺の法線に固定**で、
指がオブジェクトの周りをどれだけ回ったか（画面上の掃引角）だけを見る。
1フレームで四分の一回転を超える報告は、指が中心を跨いで方位が反転しただけなので
棄却する。指を離しても慣性で回り続け、減衰して止まる。
**ハンド操作を有効にしていない間は、同じ回転を画面のスワイプで行える**
（オブジェクトの上から始めたドラッグのみ。ボタンやリンクの上では無効）。
つまむ判定・持ち上げ・落下は廃止した。オブジェクトはマーカー平面から
垂直に少し浮かせて表示する。

**手の可視化と接触**: 描くのは人差し指（付け根〜先端）だけ。触れている間は赤くなる。
