...
1. **オブジェクトをアンカーの子にしない。** MindAR はロスト時に
   ...
   最後の姿勢のまま残る。
2. **手が映った時点でトラッカーを止める。** 手は名刺を覆いながら
   入ってくるので、ピンチ検知を待つと既にロストしている。手の検出を
   合図に `controller.stopProcessVideo()`、手が一定フレーム離れたら
   `processVideo()` で再同期する。

**手の可視化と接触**: 検出した21点を `#overlay` の 2D キャンバスに重ねて描く。
...
触れている間はオブジェクト側を強調（リングと稜線の不透明度、発光、わずかな拡大）し、
**接触しているときだけ掴める**。

`/xr/?debug=1` で、手の検出・ピンチ・接触・ひねり角・オブジェクトの画面位置と
半径を表示する。iOS では開発者ツールが使えないので、実機調整はこれを見て行う。
