...
**姿勢の平滑化**: トラッカーは描画とは別の非同期ループで動き、完了のたびに
少しずつ違う姿勢を返す。手持ちの数ピクセルの揺れが姿勢推定では大きく増幅され、
そのまま追従するとオブジェクトががくつく（手ブレを加えた映像で実測すると
画面上で最大 22px/フレーム跳ねた）。`poseSmoother` で位置・回転・スケールを
指数平滑して追従させ、平均 10.8 → 3.9px/f、最大 22 → 6.2px/f に落としている。
平滑化の係数は毎秒あたりで表現しているので、フレームレートに依存しない。
初回の検出と再出現時は平滑化せず即座に合わせる。

`?debug=1` の `fps` と `jitter` が、実機でこの値を見るための表示。
...
カウントするのは `/xr/` を開いた時点で、カメラを起動したかどうかは問わない。
エンジンとモデルを読み終えた直後に走らせるので、表示を待たせることはない。
失敗は握り潰す。ページの動作には一切影響しない。

数値は `/xr/` の開始パネルに出る（通算・端末数・この端末からの回数）。

集計は **`/xr/stats/`** で見られる（総アクセス数・ユニーク端末数・この端末からの
回数と初回/最終日時）。このページは読むだけで、開いてもカウントは増えない。
`noindex`、`/xr/` からのリンクも張っていない。
